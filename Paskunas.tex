...
\begin{align*}
	...
	\langle \Fr_w\rangle \langle \Fr_{\bw}\rangle^{-1})
	\euF^\circ_{\fn},\\
	...
	\langle \Fr_{\bar{\fl}}\rangle V_\fl)
	\euF^\circ_{\fn}.
\end{align*}
...
\langle \Fr_\fl\rangle^{-1}
\langle \Fr_{\bar{\fl}}\rangle)
(1-\hat{\chi}_\circ^{-2}(\Fr_\fl)\cdot 
\langle \Fr_\fl\rangle^{-1}
\langle \Fr_{\bar{\fl}}\rangle)\in \Lambda_{\fs}$
when $\ell\nmid \fs$.
...
$\hat{\chi}_\circ^{-1}(\Fr_\fl)\cdot
\langle \Fr_\fl\rangle\langle \Fr_{\bar{\fl}}\rangle^{-1}$
...
For $U^p=K(\fn)^p$,
let $\TT(\fn,\oo)$ be the big Hecke algebra
acting on $M(U^p)$ as defined in \eqref{eq:completed_coh}.
By Lemma \ref{lem:coh_to_ord},
there exists a homomorphism
$\TT(\fn,\oo)\to \TT^{\ord}(\fn,\oo)$.
By abuse of notation, we write 
$\lambda_{\fn}\colon \TT_{\fn}\to \TT^{\ord}(\fn,\oo)\to
\Lambda_{\fs}$ be the composition.
Let $\Psi\colon \Gal_\K\o \TT_{\fn}$ be the 
...
\[
	...
	\hat{\chi}_\circ^{-1}
\]
modulo the maximal ideal of $\Lambda_{\fs}$.
Assume 
...
Let $\TT_{\fn}$ be the localization
of $\TT(\fn,\oo)$ at the maximal ideal 
associated to $\lambda_{\fn}$ and
$\Psi_{\fn}$ be the localization
of $\Psi$ at the maximal ideal associated to $\lambda_{\fn}$.
We have the commutative diagram
\[
\begin{tikzcd}
	\Gal_K\arrow[d,equal]\arrow[r]  &
	\TT_{\fn\ell} \arrow[d] \arrow[r] &
	\Lambda_{\fs\ell} \arrow[d]\\
	\Gal_K\arrow[r]  &
	\TT_{\fn} \arrow[r] &
	\Lambda_{\fs}
\end{tikzcd}
...
\begin{defn}
We pick $z\in D_w\cong \Gp$ as in the previous subsection
and dfine $x_\fn(\sigma,\tau)$.
We may assume  there are 
$\sigma_0,\tau_0\in D_w$
such that $ x=x_{\fn}(\sigma_0,\tau_0)$
is the image of a generator of the reducibility ideal 
$x$ for the Global pseudo-representation at  $D_w$.
We define 
\[
	q_{\fn}=
	\{x_\fn(\sigma,\tau)\mid\sigma\in\Gal_K, \tau\in D_w\}
\]
Then $\fq_{\fn}\subset \ker(\lambda_{\fn})$
...
We let $\Hom_{\Lambda_{\fs}}(S^{\ord}(\fn,\Lambda_\fs),
\Lambda_{\fs})$
play the roles of $M_2'$.
Let  $\Theta=B_{\fn}(*,U_{\fs}^{-1}\euF^{\circ}_\fs)$,
and $\lambda\colon \TT\to \Lambda_{\fs}$.
...
Let $\fq$
be generated by  $x(\sigma,\tau)$
for  $\sigma\in \Gal_\K$
and  $\tau\in \Gp$.
We pick $\sigma_0, \tau_0\in \Gp$
such that  $x(\sigma_0,\tau_0)=x$
is a generator of the reducibility ideal of $R$.
...
$y_{\fn}(\sigma)\otimes \euF^\circ_{\fn}$
lifts to $M(\sigma)$
such that 
 $y_{\fn}(\sigma)\euF^\circ_{\fn}-xM(\sigma)\in 
 \fq_{\fn}S_{\fn}$.
We define $c(\sigma)=\Phi(M(\sigma))\in \Lambda_{\fs}$.
