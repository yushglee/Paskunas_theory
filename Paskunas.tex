...
\[
	...
	\otimes_{\sigma\in I_w}\xi_{k_\sigma}(g_v)
\]
...
such that $U_p=K_p$.
For $w\in \Sigma_p$ and $c\geq b\geq 0$ with  $c>0$, 
...
we then write $\Iw(p^{b,c})$
for $\prod_{v\in S_p}\Iw(w^{b,c})$.
We recall from \cite[\S 2.3]{ger}
the collection of Hecke operators 
acting on $S_{\wt{k}}(U^p\Iw(p^{b,c}),M)$
defined as the following double-coset operators.
...
be a standard parabolic subgroup containing 
the Borel subgroup $B_w\coloneqq B_n(\oo_w)$,
and write
$P=\prod_{w\in \Sigma_p}P_w$,
$B=\prod_{w\in \Sigma_p}B_w$.
Let $P=QU$
and $B=TN$ denote the Levi decompositions and define
...
\[
	...
	\}.
\]
Then when $\alpha_{w}^{(j)}\in Z_Q$,
the center of the Levi subgroup $Q$,
by a slight abuse of notation 
we also write $U_{\wt{k},w}^{(j)}$
for the similarly defined double-coset operator
on $S_{\wt{k}}(U^p\Iw^P(p^{b,c}),M)$.
As explained in \cite[Lem 2.10]{ger},
these operators commute with each other 
and are equivariant with respect to the inclusions
$ S_{\wt{k}}(U^p\Iw(p^{b,c}),M)\hookrightarrow
S_{\wt{k}}(U^p\Iw(p^{b',c'}),M)$
if $b'\geq b$ and $c'\geq c$.



\begin{defn}
	Givne a parabolic subgroup $P$ as above,
	and let  $M$ be either a finite  $\oo$-module
	or the Pontryagin dual of which, 
	we define the space of $P$-ordinary modular forms as
	\[
	S_{\wt{k}}^{P-\ord}(U^p\Iw^P(p^{b,c}),M)\coloneqq
		e_PS_{\wt{k}}(U^p\Iw^P(p^{b,c}),M),\quad
		e_P\coloneqq\lim_{n\to \infty}
		(\prod_{w\in \Sigma_p}\prod_j
		U_{\wt{k},w}^{(j)})^{n!}
	\]
	here $j$ runs through indices $1\leq j\leq n$
	such that  $\alpha_w^{(j)}\in Z_Q$,
	the center of the Levi subgroup $Q\subset P$.
	In particular,
...
let $U_0=U\cap K_p$,
$Q^+=\{m\in Q\mid mU_0m^{-1}\subset U_0\}$, and 
$Z_Q^+=Q^+\cap Z_Q$.
For a $P$-representation $V$ define
\[
	h_Q(m)\cdot v=\sum_{x\in U_0/mU_0m^{-1}} xm\cdot v
	\text{ for } v\in V^{U_0}.
\]
Then the functor of ordinary parts
$ \Ord_P\colon \Mod^{\sm}_{G_p}(A)\to \Mod^{\sm}_Q(A)$
is defined as
\[
	\Ord_P(V)=\Hom_{A[Z_Q^+]}(A[Z_Q], V^{U_0})_{Z_Q-\fin}
\]
