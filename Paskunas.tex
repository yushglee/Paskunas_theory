...
let $W_\lambda=\otimes_{I_\F}W_{\lambda_\sigma}$ denote the algebraic  
$\prod_{I_\F}\GL(n)$-representation
of highest weight $\lambda$.
Then  $K_p$ acts on  $W_{\lambda, A}$ via 
\[
	K_p=\prod_{v\in S_p}\GL_n(\oo_v)\to
	\prod_{v\in S_p}\prod_{v_\sigma=v}\GL_n(A)=\prod_{\sigma}\GL_n(A)
\]
where $\GL_n(\oo_v)\mapsto \prod_{v_\sigma=v}\GL_n(A)$
is the diagonal map.
And for $U$ as above, 
we write  
$S_{\tau}(U,A)=S_{\lambda}(U,A)$
and $S_{\tau,\psi}(U,A)=S_{\lambda,\psi}(U,A)$
when $W_\tau=W_\lambda$;
and $S_{\lambda}(U,A)=S(U,A)$, 
$S_{\lambda,\psi}(U,A)=S_\psi(U,A)$
when $\lambda$ is the trivial weight.
...
\begin{defn}
	Recall that when 
	$P=MN$ is the Levi decomposition of 
	a parabolic subgroup in the connected reductive $p$-group  $G_p$,
	and $A$ is a complete Noetherian local  $\oo$-algebra
	...
	\]
	where $M_0=M(\oo_p)=K_p\cap M$ and  $N_0=K_p\cap N$.
	The functor preserves admissibility.

	Assume $\psi(U^p\cap Z_\GG(\A_f))=1$, then
	$S_\psi(U^p,E/\oo)= \varinjlim_{U_p}S_\psi(U^pU_p,E/\oo)$
	is a smooth admissible $G_p$-representation under 
	right translation action,
	where $\{U_p\}$ is the filtered system of 
	all the compact open subgroups in $K_p$ under inclusion relation.
	We consider
	\begin{equation}
	\Ord_P(S_\psi(U^p,E/\oo))\in \aMod_{M}(\oo).
	\end{equation}
\end{defn}
...
\begin{proof}
	...
	\begin{multline*}
		...
		\Hom_{\oo[M_0]}(\pi,
		\Hom_{\oo[Z_M^+]}
		(\oo[Z_M], S_\psi(U^p,E/\oo)^{N_0})_{Z_M-\fin})\\=
		\Hom_{\oo[M_0]}(\pi,
		\Hom_{\oo[Z_M^+]}
	...
	\end{multline*}
	where the second equality follows from the $\oo$-finiteness of $\pi$,
	...
	Write $\tau=\cInd_{M_0N_0}^{K_p}\pi, \tau_1=\cInd_{M_0N_0}^{K_p}\pi_1$
	and observe that 
	\[
	\Hom_{\oo[M_0]}(\pi, S_\psi(U^p,E/\oo)^{N_0})\cong
	\Hom_{\oo[K_p]}(\tau, S_\psi(U^p,E/\oo))\cong 
	S_{\tau^\vee, \psi}(U^pK_p).
	\]
	The proposition now follows from that 
	$S_{\tau^\vee, \psi}(U^pK_p)\twoheadrightarrow 
	S_{\tau_1^\vee, \psi}(U^pK_p)$
	by Lemma \ref{lem:small}
	and that by loc.cit
	\[
		\Hom_{\oo[Z_M^+]}(\oo[Z_M], S_{\tau^\vee, \psi}(U^pK_p))
		\cong \prod_{\fm:\text{ordinary}}
		S_{\tau^\vee, \psi}(U^pK_p)_{\fm}.
	\]
\end{proof}
