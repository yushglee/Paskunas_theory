...
Recall that 
$\TT^P(U^p,\oo)$ acts faithfully on
$\Ord_P(S(U^p,E/\oo))\in \Mod_Q^{\adm}(\oo)$
and $M(U^p)=\Ord_P(S(U^p,E/\oo))$.
And there exists 
...
To apply the results in the previous subsection,
we follow Urban and make the following twist.
Let $\Psi\colon \Gp\to R$ be 
...
and induces $R^{\epsilon\zeta}\to R$,
where $R^{\epsilon\zeta}$ is the universal 
...
\]
Note that the action of the center $\Qp^\times\subset \GL_2(\Qp)\subset Q$
on  $\Ord_P(S(U^p,E/\oo))_{\fm}$ factors through  $\TT(U^p,\oo)_\fm$,
and is precisely given by
\[
	\epsilon\det\Psi_\fm\circ \Art\colon\quad
	p\mapsto U_w^{(2)},\quad
	x\mapsto h_U(x)\,x\in\Zp^\times
\]

\begin{defn}
	Recall that $Q$ is the product 
	of  $\GL_2(\Qp)$ and the center  $Z_Q$.
	We let
	$\Ord_P(S(U^p,E/\oo))_{\fm}'\coloneqq
	\Ord_P(S(U^p,E/\oo))_{\fm}(\xi_\fm^{1/2})$
	denote the $\GL_2(\Qp)\times Z_Q$-representation
	with the same $Z_Q$-action
	and the $\GL_2(\Qp)$-action 
	twisted by $\det\circ \xi_\fm^{1/2}$.
	We also let $M(U^p)_\fm', S(U^p)_\fm'$
	denote the twisted variant.

	We note that 
	$\Ord_P(S(U^p,E/\oo))_{\fm}'$
	is still admissible as  $\GL_2(\Qp)\times Z_Q$-representation.
	The restriction of which to $\GL_2(\Qp)$
	is therefore locally admissible,
...
\begin{proof}
	...
	Let $T\colon \Gp\to R^{\epsilon\zeta}$ 
...
Let $\fm\subset \TT^P(U^p,\oo)$
be the maximal ideal satisfying  \eqref{cond:red_gen}
as before.
Since $Z_Q$ acts 
on  $\Ord_P(S(U^p,E/\oo))$
through the Hecke operators,
the action 
factors through  $\TT^P(U^p,\oo)$
and we let  $\chi\colon Z_Q\to \TT^P(U^p,\oo)/\fm=\fF^\times$
denote the residue character. 

Let $\B=\{\pi_1,\pi_2\}$ 
be the block of irreducible $\GL_2(\Qp)$-representation
defined as before.
Let $Z_Q$ acts on  $\pi_i$ by  $\chi$.
By Proposition \ref{prop:compatibility},
the irreducible subquotients
of the $\GL_2(\Qp)\times Z_Q$-representation
of  $\Ord_P(S(U^p,E/\oo))_{\fm}'$
are isomorphic to either $\pi_1$ or  $\pi_2$.
...
We denote by $\tilde{P}_\chi$
the projective envelope of  $\chi^\vee$
and  $ \tilde{E}_\chi\coloneqq \End_{\fC_{Z_Q}(\oo)}(\tilde{P}_\chi)$.
After twist, we may assume $\chi$ is the trivial character,
then the same argument in 
\cite[Prop 3.34]{pask}
shows that
$\tilde{E}_\chi$
is the formal power series ring over $\oo$ 
of relative dimension $\dim\Hom^{\cont}(Z_Q,\fF)$
and $\tilde{P}_\chi$ is free of rank one over $\tilde{E}_\chi$.


\begin{prop}\label{prop:envelope}
	Let $R_{\fm}\coloneqq R\hat{\otimes}_{\oo}\tilde{E}_\chi$.
	The $R_{\fm}$-module
	$\tilde{P}_i\hat{\otimes}_{\oo}\tilde{P}_\chi$
	is the projective envelope
	of the $\GL_2(\Qp)\times Z_Q$-representations $\pi_i$
	and $\End_{\GL_2(\Qp)\times Z_Q}
	(\tilde{P}_i\hat{\otimes}_{\oo}\tilde{P}_\chi)\cong R_{\fm}$.
\end{prop}
\begin{proof}
	Suppose first that
	$V\in \Mod_{\GL_2(\Qp)\times Z_Q}(\oo)$
	is locally admissible
	and $v\in V$ is an admissible vector.
	Then  $v$ is  $Z_Q$-finite by \cite[Lem 2.3.5]{emeI}
	and the $\oo[\GL_2(\Qp)\times Z_Q]-module$
	generated by $v$ is also finitely-generated over  $\GL_2(\Qp)$
	and admissible.
...
\begin{cor}\label{cor:Hecke_Noetherian}
	...
	and $\mathbf{m}\coloneqq
	\Hom_{\GL_2(\Qp)\times Z_Q}(\tilde{P}_{\fm},M(U^p)_{\fm}')$
	is a finite faithful $\TT$-module,
	where $\tilde{P}_{\fm}\coloneqq (\tilde{P}_{1}\oplus \tilde{P}_{2})
	\hat{\otimes}_{\oo}\tilde{P}_\chi$.
\end{cor}
\begin{proof}
	Since $\Ord_P(S(U^p,E/\oo))_\fm'$
	is admissible
	and all irreducible subquotients of which
	belong to $\{\pi_1,\pi_2\}$.
	It follows from 
	\cite[Prop 4.17]{pask}
	that $\mathbf{m}$ is finitely-generated
	over $R_{\fm}$.
	Since the action of $R_{\fm}$
	factors through $\TT(U^p,\oo)_{\fm}$,
...
\begin{lem}
	...
	0\to \tilde{P}_\fm^r\to \tilde{P}_\fm^r\to M(U^p)_{\fm}'\to 0
	\end{equation}
\end{lem}
\begin{proof}
Identify $\Qp^\times$ with the center of $\GL_2(\Qp)$.
By \cite[Thm 33]{barthel} and \cite[Thm 19]{barthel}, there exists
smooth irreducible $\GL_2(\Zp)\Qp^\times$-representations $\sigma_i$
and exact sequences
\begin{equation}
	0\to 
	...
	\pi_i\to 0
\end{equation}
Write $M=M(U^p)_{\fm}'$ for short.
Let $Z_Q$ acts on above by  $\chi$
and take $\Ext^i(*,M)$ gives
\begin{equation*}
    \begin{tikzcd}[row sep=2ex]
	    \Ext^{i-1}_{G}(\pi, M)\arrow[r] &
	    \Ext^{i}_{G}(\text{c-ind}_{KZ}^G\sigma, M)\arrow[r] \arrow[d,symbol={=}] &
	    \Ext^{i}_{G}(\text{c-ind}_{KZ}^G\sigma, M)\arrow[r] \arrow[d,symbol={=}] &
	    \Ext^{i}_{G}(\pi, M)\\ 
												 & \Ext^i_K(\sigma ,M) &
	    \Ext^i_K(\sigma ,M) &
    \end{tikzcd}
\end{equation*}
Since $M$ is injective as a $K$-representation, 
the long exact sequence reduces to 
\begin{equation*}
	0 \to \Hom_G(\pi_i,M)\to \Hom_K(\sigma_i,M)\to \Hom_K(\sigma_i,M)\to \Ext^1_G(\pi_i,M)\to 0
\end{equation*}
It follows that $a_i\coloneqq \dim_k\Hom_G(\pi_i,M(U^p)_{\fm}')=\dim_k \Ext^1_G(\pi_i,M)$,
which are finite since $M$ is admissible.
...
through the isomorphisms in \eqref{eq:end_deform}.
Denote by $\bar{A}_{ij}\in M_r(R^{\red})$
the reduction of the matrices modulo $\tau$.
Lemma \ref{lem:ker_red} shows that 
