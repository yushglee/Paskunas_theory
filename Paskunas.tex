...
\begin{defn}
	...
	of $M$ given by
	\begin{equation}\label{eq:algrepn}
...
\begin{cor}\label{cor:wt_indep}
	Define $S_{\wt{k}}^{P-\ord}(U^p,A)$ 
	as $\lim_{b\to \infty}
	S_{\wt{k}}^{P-\ord}(U^p\Iw^P(p^{b,b}),A)$.
	We have the following isomorphism
	that are equivariant with respect to the 
	Hecke operators and the $Q_0$-action 
	defined in previous proposition.
	\[
		\epsilon_{\wt{k}} \colon 
...
after taking the injective limit.
\end{proof}
...
$U^p$ is such that $U^pK_p$ is sufficiently small.
Let  $S(U^p,E/\oo)$ be as in \eqref{eq:complete}
and $P=QU\subset G_p$ be a parabolic subgroup, 
recall that 
$\Ord_P(S(U^p,E/\oo)\in \Mod_{Q}^{\adm}(\oo)$.
...
Write $S(U^p)_E=S(U^p)\otimes_\oo E$,
which is an  $E$-Banach space
with $S(U^p)$ as a unit ball.
We define the subspace $S^{\alg}(U^p)_E$ 
of algebraic vectors for 
$Q_0=Q\cap K_p$ as the image of 
\[
\bigoplus_{\wt{k}}\Hom_{E[Q_0]}(\pi_{\wt{k}}^*(E), S(U^p)_E)
\otimes_E \pi_{\wt{k}}^*(E)\rightarrow S(U^p)_E
\]
where $\wt{k}$ ranges through all dominant weights.

\begin{prop}\label{prop:density}
	The subspace $S^{\alg}(U^p)_E$
	is dense in $S(U^p)_E$
\end{prop}
\begin{proof}
	Since $\Ord_P(S(U^p,E/\oo))$ is an injective object
	in $\Mod_{Q_0}^{\sm}(\oo)$
	by Proposition \ref{prop:inj},
	...
	\cite[Prop 6.A.17]{pask}.
\end{proof}

\begin{lem}\label{lem:wt_indep}
	Write $S_{\wt{k}}^{P-\ord}(U^p\Iw^P(p^{b,c}),E)=
	S_{\wt{k}}^{P-\ord}(U^p\Iw^P(p^{b,c}),\oo)\otimes_\oo E$,
	then there exists a Hecke-equivariant
	\[
...
\end{lem}
\begin{proof}
	By corollary \ref{cor:wt_indep},
	there exists a $Q_0 $-isomorphism
	\[
	...
	is the  $Q_0$-invariant subspace of the right hand side.

	On the other hand, as 
	$\Hom_\oo(E/\oo, S_{\wt{k}}^{P-\ord}(U^p,E/\oo))\cong
	\varprojlim_r S_{\wt{k}}^{P-\ord}(U^p,\oo/(\varpi^r))=
	S_{\wt{k}}^{P-\ord}(U^p,\oo)$,
	the  $Q_0$-invariant subspace of the left hand side
	is isomorphic to 
	$S_{\wt{k}}^{P-\ord}(U^p\Iw^P(p^{0,1}),\oo)$
	by lemma \ref{lem:control}.
	The claimed result now follows by
	taking the $Q_0$-invariant subspaces 
	of the above isomorphism and $\otimes_\oo E$.
\end{proof}
...
The following lemma is a restatement 
of \cite[Cor 2.33]{ger}
and \cite[Lem 2.31]{ger} in the our situation.

\begin{lem}
	Let $\pi$ be an irreducible constituent of 
	$S_{\wt{k}}(\bar{Q}_p)$ and suppose 
	$\pi\cap S_{\wt{k}}^{P-\ord}(U^p\Iw^P(p^{0,1}),E)\neq 0$
	\begin{enumerate}[label=(\alph*)]
		\item If $\wt{k}$ is sufficiently regular,
			then $r_\pi$ is crystalline  at $w'\in \Sigma_p$
			and  $w'\neq w$;
			and there exists characters
			 $\psi_i\colon G_{\K_{w'}}\to E^{\times}$ such that
		\[
			r_\pi\vert_{G_{\K_{w'}}}\sim
			\begin{pmatrix}
				\psi_1 &*\\&\epsilon^{-1}\psi_2
			\end{pmatrix}\quad
			\begin{aligned}
			\psi_1\circ \Art&\colon & 
			&\varpi_{w'}\mapsto u_{w'}^{(1)}&
			&x \mapsto 
			\prod_{\sigma\in I_{w'}}
			\iota_p(\sigma(x))^{-k_{2,\sigma}}\,
			x\in \oo_{w'}^\times \\
			\psi_1\circ \Art&\colon & 
			&\varpi_{w'}\mapsto (u_{w'}^{(2)})/(u_{w'}^{(1)})&
			&x\mapsto 
			\prod_{\sigma\in I_{w'}}
			\iota_p(\sigma(x))^{-k_{1,\sigma}}\,
			x\in \oo_{w'}^\times\\
			\end{aligned}
		\]
		where $u_{w'}^{(j)}$ are the eigenvalues of 
		$U_{\wt{k},w'}^{(j)}$ on 
		$\pi\cap S_{\wt{k}}^{P-\ord}(U^p\Iw^P(p^{0,1}),E)\neq 0$.
	\item The representation $r_\pi$ is crystalline at $w$.
	If furthermore $\pi\cap S_{\wt{k}}^{B-\ord}(U^p\Iw(p^{0,1}),E)\neq 0$,
	then $r_\pi\vert_{G_{\K_w}}$ is conjugate
	to an upper triangular representation as above.
	\end{enumerate}
\end{lem}
...
\begin{defn}
	...
	or the Pontryagin dual of which,
	let $\TT_{\wt{k}}(U^p\Iw^P(p^{b,c}),M)$
	be the $\oo$-subalgebra
	of $\End_{\oo}(S^{P-\ord}_{\wt{k}}(U^p\Iw^P(p^{b,c}),M))$
	generated by $T_w^{(j)}$ and $U_{\wt{k},w'}^{(j)}$
	and $U_{\wt{k},w}^{(2)}$ and $\langle u\rangle$.
	We also define 
	\[
		\TT_{\wt{k}}(U^p,E/\oo)=\varprojlim_b
		\TT_{\wt{k}}(U^p\Iw^P(p^{b,b}),E/\oo)\quad
		\TT_{\wt{k}}(U^p,\oo)=\varprojlim_b
		\TT_{\wt{k}}(U^p\Iw^P(p^{b,b}),\oo)
	\]
	In particular,
	$\TT_{\wt{k}}(U^p\Iw^P(p^{b,c}),\oo)$
	are reduced finite flat  $\oo $-algebras
	by \cite[Lem 2.14]{ger}.
	Consequently 
	$\TT_{\wt{k}}(U^p,\oo)$,
	which is isomorphic to $\TT_{\wt{k}}(U^p,E/\oo)$
	by \cite[Lem 2.17]{ger},
	is also reduced 
	and acts faithfully
	on $S_{\wt{k}}^{P-\ord}(U^p,E/\oo)$.
	We write 
	$\TT_{\wt{k}}(U^p,E/\oo)=\TT(U^p,E/\oo)$
	when $\wt{k}$ is the trivial weight. 
\end{defn}
By lemma \ref{lem:wt_indep}, 
there exists homomorphisms 
$\TT(U^p,\oo)\to \TT_{\wt{k}}(U^p\Iw^P(p^{0,1}),\oo)$.
Thus for $\pi$ such that
$\pi\cap S_{\wt{k}}^{P-\ord}(U^p\Iw^P(p^{0,1}),\oo)\neq 0$,
we may define the prime ideal $\fp_\pi\subset \TT(U^p,\oo)$
associated to the Hecke-eigensystem
$\lambda_\pi\colon \TT_{\wt{k}}(U^p\Iw^P(p^{0,1}),\oo)$.


Since $\TT_{\wt{k}}(U^p\Iw^P(p^{b,b}),\oo)$ is reduced,
when  $\pi$ goes through 
irreducible constituents of $S_{\wt{k}}(\bar{\Q}p)$
such that
$\pi\cap S_{\wt{k}}^{P-\ord}(U^p\Iw^P(p^{b,b}),\oo)\neq 0$,
we have
\[
	\TT_{\wt{k}}(U^p\Iw^P(p^{b,b}),\oo)\hookrightarrow
	\prod_{\pi}
	\TT_{\wt{k}}(U^p\Iw^P(p^{b,b}),\oo)/\wp_{\pi}
\]
Since for each $\pi$ as above,
the associated Galois representation
$r_{\pi}$ satisfies 
$\mtr(r_{\pi})(\Fr_w)=\lambda_{\pi}(T_w^{(1)})$.
By Chebotarev density, there thus exists 
a pseudo-representation
\[
	\Psi_{\wt{k}}(U^p\Iw^P(p^{b,b}))\colon G_{\K}
	\to \TT_{\wt{k}}(U^p\Iw^P(p^{b,b}),\oo)\quad
	\Fr_w\mapsto T_w^{(1)}
\]
Taking the inverse limit, 
we obtain the big Galois pseudo-representation
\[
	\Psi\colon G_{\K}\to 
	\TT(U^p,\oo) \quad
\]
Furthermore, by Lemma \ref{prop:density},
the prime ideals $\wp_\pi$
such that
$\pi\cap S_{\wt{k}}^{P-\ord}(U^p\Iw^P(p^{0,1}),\oo)\neq 0$
are dense in $\TT(U^p,\oo)$
and the associated $r_\pi$
are all reducible at  $w'\in \Sigma_p$ for  $w'\neq w$
Thus  $\Psi$ is  also reducible at all $w'\in \Sigma_p$.
