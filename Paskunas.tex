...
\begin{proof}
	...
	the  $\Iw^P(p^{1,0})$-invariant subspace is one-dimensional, with
	 \[
		U_1=q\chi_1(\varpi)+\chi_2(\varpi),\quad
		U_2=\chi_1\chi_2(\varpi)\quad
		U_3=q^{-1}\chi_1\chi_2\chi_2(\varpi)
	\]
	Thus 
	\begin{gather}
		\val(\chi_2(\varpi_w))=\sum_{\sigma\in I_w}k_{\sigma,1}+1\\
		\val(\chi_1(\varpi_w))=\sum_{\sigma\in I_w}
		k_{\sigma,3}+k_{\sigma,2}-k_{\sigma,1}-1
		\sum_{\sigma\in I_w}(k_{\sigma,2}-k_{\sigma,1})+
		\sum_{\sigma\in I_w}(k_{\sigma,1}-k_{\sigma,3})+1
	\end{gather}
	but the last leads to contradiction when $\wt{k}$
	...
	*&*&*\\
	&*&*\\
	&*&*
	\end{psmallmatrix}$ follows using inner involution.
\end{proof}
