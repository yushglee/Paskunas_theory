...
\begin{proof}
	...
	for $j=1,2$.
	From the proof of Lemma \ref{lem:ker_red}
	we may assume $x=\phi_{ij}\circ \phi_{ji}$ is injective
	on $\tilde{P}_{i,\chi}$ for both $i=1,2$.
	Apply the snake lemma the diagra
    \begin{equation*}
    \begin{tikzcd}
...
which is trivial by the proposition.
\[
	\ker\left(
...
Aslo write 
$R_{\fm}=R_{\fm,\chi}$,
$\Lambda_{\fm}=\oo\llbracket T(\oo)^\wedge\rrbracket$
and $\TT_{\fm}$ be the image of $\TT(U^p,\oo)_{\fm}$
in $\End_{R_\fm}(M)$.
Then $\TT_{\fm}$ and $R_{\fm}$ are $\Lambda_{\fm}$-modules.
By the above corollary
and the admissibility of $M^{\ord}(U^p)_{\fm}$
we see $M^{\red}$ is also a finite $\Lambda_{\fm}$-module.

We now assume that 
there exists a nonzero homomorphism
$\lambda\colon \TT_{\fm}\to \Lambda_{\fm}$
of $\Lambda_{\fm}$-algebras
and a nonzero homomorphism
$\Theta\colon M_2\to \Lambda_{\fm}$
of $\Lambda_{\fm}$-modules such that
\[
	\Theta(T\cdot m)=\lambda(T)\cdot \Theta(m)
	\text{ for } T\in \TT_{\fm}, m\in M_2.
\]
Let $\fq\subset \ker(\TT_{\fm}\to \End(M_1))$
be some ideal containing the image of a generator $x$ 
of the reducibility ideal in $R_{\fm}$,
and let $\wp=\ker\lambda\subset \TT_{\fm}$.
We assume 
\begin{equation}\label{cond:torsion}
	\fq M_2/\fq S_2 \text{ is $\Lambda_{\fm}$-torsion for }
	S_2\coloneqq\ker(\Theta)
\end{equation}

\begin{lem}
Let $S=\ker(M\to M^{\red}\to M_2
\xrightarrow{\Theta}\Lambda)$, 
then there exists an exact sequence
\begin{equation}\label{eq:fund}
	M/S\xrightarrow{x} \fq M/\fq S\to \fq M_2/\fq S_2 \to 0
\end{equation}
\end{lem}
\begin{proof}
...
Since $\Theta\colon M/S\cong M_2/S_2\hookrightarrow \Lambda_{\fm}$,
the $\TT_{\fm}$-actions on \eqref{eq:fund}
all factor through  $\lambda\colon \TT_{\fm}\to \Lambda_{\fm}$.
Let $\mathbb{K}=(\Lambda_{\fm})_\wp$
be the field of fraction of $\Lambda_{\fm}$
and apply 
$\otimes_{\TT_{\fm}}\TT_{\wp}=
\otimes_{\Lambda_{\fm}}(\Lambda_{\fm}\otimes_{\TT_{\fm}}\TT_\wp)=
\otimes_{\Lambda}\mathbb{K}$ 
to the sequence \eqref{eq:fund}.
As $\fq M_2/\fq S_2$ is $\Lambda_{\fm}$-torsion, 
...
\begin{proof}
	...
	which contains $\mathbb{K}\llbracket x\rrbracket$
	as a subring.
	Thus
	\[
		\mathbb{K}\llbracket x\rrbracket\hookrightarrow
	R_\wp \longrightarrow T_\wp
	\text{ acts on } M_\wp
	\]
	In fact, the composition
	$\mathbb{K}\llbracket x\rrbracket\to \TT_\wp$
	is injective 
	...
	and moreover 
	$\dim_\mathbb{K} M_\wp/xM_\wp=(M_1\oplus M_2)_\mathbb{K}<\infty$.
	Thus
	$M_\wp$ is finite over $\mathbb{K}\llbracket x\rrbracket$
	by the topological Nakayama lemma,
	from which the lemma follows as
	$\TT_\wp\hookrightarrow 
	\End_{\mathbb{K}\llbracket x\rrbracket}(M_\wp)$.
\end{proof}
...
\end{prop}
\begin{proof}
	Since $M/S$ is isomorphic to a nonzero submodule in  $\Lambda_{\fm}$,
	...
	in $\textnormal{Frac}(\TT/\fq_i)$.
	Then each $\TT_i$ is a discrete valuation ring.
	Define $\tilde{\TT}\coloneqq\prod_i \TT_i$
	where  $\fq_i$ goes through all minimal primes.
