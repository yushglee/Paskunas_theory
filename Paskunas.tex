...
act on 
$M=\Hom(P\hat{\otimes})\Lambda', M(U^p)_\fm)$.
...
and morphism $\Theta\colon M_2\to \Lambda$ 
such that 
\[
...
\begin{enumerate}[label=(\alph*)]
	\item $\Theta$ is surjective?
	\item $\fq M_2/\fq S_2$ is $\Lambda$-torsion.
	\item  $\TT$ is reduced
	\item $R\times \Lambda'\to \Lambda$
		is of codimension two.
	\item $\TT$ acts faithfully(flat) on  $M$.
\end{enumerate}
...
\end{tikzcd}
\Longrightarrow
M/S\to \fq M/\fq S\to \fq M^{\red}/\fq S^{\red}
\to 0
\]
Note that $\fq M^{\red}=\fq M/xM$ 
inject into $M^{\red}=M/xM$ and 
the image of $S^{\red}=S/xS$ into $M^{\red}$ 
is $S+xM/xM=M_1\oplus S_2$, thus
\[
	Im(\fq S^{\red}\to M^{\red})=\fq S+xM/xM=\fq S_2
\]
we conclude that $\fq M^{\red}/\fq S^{\red}=\fq M_2/\fq S_2$.
\end{proof}

We see that among the modules in \eqref{eq:fund},
$\Theta(M/S)=\Theta(M_2/S_2)\subset \Lambda$,
$\fq\otimes_{\TT}M/S\twoheadleftarrow \fq M/\fq S$,
so all $\TT$-actions
factors through  $\lambda\colon \TT\to \Lambda$.
Let  $\Lambda_\wp\cong K_\Lambda$
be the field of fraction.
Note that 
$\otimes_{\TT}\TT_{\wp}=
\otimes_\Lambda(\Lambda\otimes_\TT\TT_\wp)=
\otimes_\Lambda K_\Lambda$,
by the exact sequence \eqref{eq:fund}
and the  $\Lambda$-torsion of $\fq M_2/\fq S_2$,
we get 
\[
	K_\Lambda\twoheadleftarrow (\fq M/\fq S)_{\wp}
	= \fq M_\wp/\fq S_\wp
\]
From which,
we see that the injectivity of \eqref{eq:fund}
...
\begin{lem}
	...
	$T_\wp\hookrightarrow \End_{K\llbracket x\rrbracket}
	(M_\wp)$
	and is finite over $K\llbracket x\rrbracket$.
\end{lem}
\begin{proof}
	Let $R_\wp$ be the localization 
	at the prime 
	$\ker(R\times \Lambda'\hookrightarrow \Lambda)$,
	then by dimension counting
	$R_\wp\cong K\llbracket x,y\rrbracket$
	where  $K=K_\Lambda$.
	Thus
	\[
	R_\wp\cong K\llbracket x,y\rrbracket
	\longrightarrow T_\wp
	\text{ acts on } M_\wp
	\]
	Thus $K\llbracket x\rrbracket$
	inject into  $\TT_\wp$,
	since  $x\neq 0$ in  $\TT_\wp$
	and the latter is reduced
	(so the image is not nilpotent),
	and  $\TT_\wp$ acts faithfully on  $M_\wp$.

	Now, combine that
	$M_\wp$ is finite over $\TT_\wp$,
	so that  $M_\wp$ is compact,
	and that  $\dim_K M_\wp/xM_\wp<\infty$,
	we have that 
...
Write $\TT=\TT_\wp$ and  $M=M_\wp$,
recall that  $K\llbracket x\rrbracket\to \TT$
is finite, the latter acts flattly on  $M$. 
Put
 \[
	\TT\hookrightarrow \tilde{\TT}=\prod_{ht \fq_i=0}
	\TT/\fq_i\quad
	M\hookrightarrow M\otimes_\TT\tilde{\TT}=
	\prod_{ht \fq_i=0}
	M/\fq_i M
\]
here the first injective follows from reducedness
of $\TT$.

The field of fraction of  $\TT/\fq_i$
is a finite extension over  $K(x))$,
let $\TT_i$ be the integral closure of  
$K\llbracket x\rrbracket$,
which contains  $\TT/\fq_i$, thus
 \[
	\TT\hookrightarrow \tilde{\TT}\hookrightarrow
	\TT'=\prod_i \TT_i \text{ product of d.v.r}\\
	M\hookrightarrow M'=M\otimes_\TT\TT'=
	\prod_i M/\fq_i\otimes_\TT\TT_i
\]

Let $S=S_\wp$, then 
 \[
	M\otimes\TT'/S\otimes\TT'\cong 
	K\otimes_{\TT,\lambda}\TT'=
	\prod_i K\otimes_{\TT}\TT_i
\]
This is nonzero since finite (integral) maps are surjective 
in spectrum level
\[
\begin{tikzcd}
	& Spec(K) \arrow[d]\\
	Spec \TT_i \arrow[r,twoheadrightarrow] &
	Spec \TT
\end{tikzcd}
\]
Now
\[
	\fq M\otimes_\TT\TT'=
	M\otimes_\TT\fq \TT'=
	M\otimes_\TT\prod_i\fq \TT_i=
	\prod_i(M/\fq_i)\otimes_\TT\fq \TT_i=
\]
Therefore we see that 
\begin{multline}
	\fq M/\fq S\otimes_\TT \TT'
	\cong \fq M\otimes_\TT\TT'/ \fq S\otimes_\TT\TT'
	\cong \prod_i
	(M/\fq_i M)/(S/\fq_i S)\otimes_\TT \fq\TT_i
	\\\cong \prod_i
	(M/\fq_i M)/(S/\fq_i S)\otimes_\TT \TT_i
	\cong M\otimes\TT'/S\otimes\TT'
\end{multline}
which we have shown to be nonzero;
the second to last isomorphism follows from that 
each $\fq\TT_i$ is a principal ideal in the dvr $\TT_i$.
We thus conclude that 
$\fq M/\fq S\neq 0$.

Note that 
\[
	\fq M/\fq S\otimes_\TT\TT'\cong
	M\otimes_\TT\fq\TT'/ S\otimes_\TT\fq\TT' \cong 
	M\otimes_\TT\TT'/ S\otimes_\TT\TT'\cong
	M/S\otimes_\TT\TT'
\]
