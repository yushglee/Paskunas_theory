...
\begin{enumerate}
    ...
    \item the prime-to-$p$ part of the conductor of 
    $\tilde{\eta}$ divides $\fs$.
\end{enumerate}
...
\begin{prop}\cite[Thm 7.6]{lee}
	Let $\fX_{\fs}^-$ denote
	the set of characters $ \hat{\eta}$ of $\fG_{\fs}^-$
	for algebraic  Hecke characters  $\eta$ as above.
...
\begin{align}
	T_w^{(1)}\cdot &\euF^\circ_{\fn}=q_w^{1/2}
	(\chi(\varpi_w)+\chi^{-1}(\varpi_w)
	\langle \Fr_w\rangle \langle \Fr_{\bw}\rangle^{-1})
	\euF^\circ_{\fn}\\
	U_{\wt{k}',w}^{(1)}\cdot &\euF^{\circ}_{\fn}=
	q_w^{1/2}\chi^{-1}(\varpi_w)
	\langle \Fr_w\rangle \langle \Fr_{\bw}\rangle^{-1})
	\euF^{\circ}_{\fn}\\
	 &\euF^\circ_{\fn\ell} \mod I_{\ell}=
	(1-q_\fl^{-1/2}\chi\tilde{\eta}^{-1}(\varpi_\fl)V_\fl)
	\euF^\circ_{\fn}.
\end{align}

\begin{thm}   
	Put $\mathcal{L}_\fn=
	B_\fn(\euF\circ_{\fs},U_\fs^{-1}\euF^\circ_{\fs})
    \in \Lambda_{\fs}$
    \begin{equation*}
        \frac{1}{\Omega_p^{2k+4}}
...
As $\euF^\circ_{\fn\ell}\mapsto (1-q_{\fl}^{-1/2}\chi\tilde{\eta}^{-1}(\varpi_{\fl})V_\fl)\euF^\circ_{\fn}$,
we have
\begin{multline*}
	B_{\fn\fl}
	((1-q_{\fl}^{-1/2}\chi\tilde{\eta}^{-1}(\varpi_{\fl})V_\fl)M_{\fn},U_{\fl\fn}^{-1}
	(1-q_{\fl}^{-1/2}\chi\tilde{\eta}^{-1}(\varpi_{\fl})V_\fl)\euF^{\circ}_{\fn})=\\
	U_{\fl}^{-1}(
	B_{\fn}(M_{\fn}, U_{\fn}^{-1}T_{\fl}^{(1)}\euF^\circ_{\fn})-\\(q_\ell+1)
	q_{\fl}^{-1/2}\chi\tilde{\eta}^{-1}(\varpi_{\fl})
	B_{\fn}(M_{fn}, T_\fl^{(2)}\euF^\circ_{\fn})
	+q_{\fl}^{-1}
	B_{\fn}(M_{fn}, T_\fl^{(1)}T_\fl^{(2)}\euF^\circ_{\fn})
\end{multline*}

\begin{multline*}
	\ell^{1/2}(\alpha+\beta)-2\ell^{-1/2}\beta^{-1}
	(\alpha\beta)(\ell+1)+
	\ell^{-1}\beta^{-2}(\alpha\beta)\ell^{1/2}(\alpha+\beta)\\=
	\ell^{1/2}(\alpha+\beta)-
	2\alpha(\ell^{1/2}+\ell^{-1/2})+
	\ell^{-1/2}(\alpha\beta^{-1})(\alpha+\beta)\\=
	-\ell^{1/2}(\alpha-\beta)+
	\ell^{-1/2}(\alpha\beta^{-1})(\alpha-\beta)=
	(\alpha-\beta)(\ell^{-1/2}(\alpha\beta^{-1})-\ell^{1/2})=(\ell^{1/2}\beta)(1-\alpha\beta^{-1}\ell^{-1})(1-\alpha\beta^{-1})
\end{multline*}
