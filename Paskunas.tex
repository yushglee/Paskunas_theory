...
\begin{proof}
	...
	\cite[Prop 3.2.4]{pan} , we aim to show the surjectivity of
	\[
		\Hom_{\oo[K]}(\pi,\Ord_P(S_\psi(U^p,E/\oo)))\to 
		\Hom_{\oo[K]}(\pi',\Ord_P(S_\psi(U^p,E/\oo)))
	\]
	when $\pi_{1}\hookrightarrow \pi$ 
	is an injective map between finite $\oo$-modules 
	in  $\aMod_{K}(\oo)$.
	Observe that by definition
	\begin{multline*}
		\Hom_{\oo[M_0]}(\pi,\Ord_P(S_\psi(U^p,E/\oo)))
		= \Hom_{\oo[M_0]}(\pi,\Ord_P
		(\varprojlim_{U_p}S_\psi(U^pU_p,E/\oo)))
	...
	and use the $\oo$-finiteness of  $\pi$ again, we have
	\begin{multline*}
	...
	S_\psi(U^pU_p,E/\oo)^{N_0}))_{Z_M-\fin}
	\end{multline*}
	When $U_p=K(n)$ and $I(n)=\bar{N}_{n}K_0N_0$,
	assume $n>>0$ and the action of  $K_0$
	factors through  $\mod p^n$, then 
	 \[
		 \Hom_{\oo[K_0]}(\pi,S_\psi(U^pU_p,E/\oo)^{N_0})=
		 \Hom_{\oo[I_n]}(\pi,S_\psi(U^pU_p,E/\oo)^{N_0})
		 \cong S_{\pi^{\vee},\psi}(U^pI_n)
	\]
	in which we extend the $K_0$-action on  $\pi$
	to  $I_n$ via the group homomorphism
	 \[
		I_n\mapsto P_0\mod p^n\mapsto M_0\mod p^n
	\]
	But $S_{\pi^{\vee},\psi}(U^pI_n)\mapsto S_{\pi_{1}^{\vee},\psi}(U^pI_n)$
	by the previous lemma.
	This proved the desired surjectivity.
\end{proof}
