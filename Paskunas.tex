...
for all  $v\in S_p$.
Identify $\GG(\F_v)$
and  $G_v\coloneqq \GL_n(\F_v)$ by the above isomorphism,
we denote 
\[
...
be an open compact subgroup and denote
$U_p=\prod_{v\notin S_p}U_v$,
$U_p=\prod_{v\in S_p}U_v$.
We always assume that $U_p\subset K_p$.
When $\tau\colon U_p\to \Aut(W_\tau)$
is a continuous representation on a finite  $A$-module  $W_\tau$,
define
\begin{align}
S_{\tau}(U,A)&=
...
When $\tau$ is the trivial representation and $\psi(U^p\cap Z_\GG(\A_f))=1$
we define $S_\psi(U^p,A)= \varinjlim_{U_p}S_\psi(U^pU_p,A)$.
In particular, 
\[
...
\begin{defn}
	...
	and $P=MN$ be the Levi decomposition of 
	a parabolic subgroup in the connected reductive  $p$-group  $G_p$.
	Write $M_0=M(\oo_p)=K_p\cap M$ and  $N_0=K_p\cap N$.
	The functor of ordinary parts is given by
	\[
		\Ord_P(V)\coloneqq 
...
\begin{proof}
	...
	is an injective morphism between $M_0$-admissible
	representations that are finite  $\oo$-modules.
	Since $\Ord_P$ commutes with inductive limits, by definition
	\begin{multline*}
		\Hom_{\oo[M_0]}(\pi,\Ord_P(S_\psi(U^p,E/\oo)))
		= \Hom_{\oo[M_0]}(\pi,
		\varprojlim_{U_p}\Ord_P(S_\psi(U^pU_p,E/\oo)))
		\\=\varprojlim_{U_p}\Hom_{\oo[M_0]}(\pi,\Ord_P
		(S_\psi(U^pU_p,E/\oo)))
	\end{multline*}
	where the last equality follows from the $\oo$-finiteness of  $\pi$.
	Unwinding the definition of  $\Ord_P$,
	\[
		\Hom_{\oo[M_0]}(\pi,\Ord_P (S_\psi(U^pU_p,E/\oo)))=
		\Hom_{\oo[M_0]}(\pi,\Hom_{\oo[Z_M^+]}(\oo[Z_M],
		S_\psi(U^pU_p,E/\oo)^{N_0})_{Z_M-\fin})
	\]
	it follows again from the $\oo$-finiteness of  $\pi$
	that any homomorphism factors through 
	$S_\psi(U^pU_p,p^{-s}\oo/\oo)$ for a sufficiently large $s$ and
	the $Z_M$-fintieness condition is automatic
	\cite[Lem 3.1.5]{emeI}.
	Thus the right hand side is isomorphic to 
	$\Hom_{\oo[M_0]}(\pi,\Hom_{\oo[Z_M^+]}(\oo[Z_M],
			S_\psi(U^pU_p,E/\oo)^{N_0}))$,
			which is isomorphic to 
	\[
		\Hom_{\oo[Z_M^+]}(\oo[Z_M],\Hom_{\oo[M_0]}(\pi,
		S_\psi(U^pU_p,E/\oo)^{N_0})).
	\]

	Now, 
	define the open compact subgroups
	$K(n)\coloneqq\{k\in K_p\mid k\equiv 1 \mod p^n\}$ and
	$I(n)\coloneqq\{k\in K_p\mid k\in P \mod p^n\}$ of $G_p$,
	the latter group admits the Iwahori decomposition
	$I(n)=\bar{N}_{n}M_0N_0$,
	where $ \bar{N}_{n}=\bar{N}\cap I(n)=\bar{N}\cap K(n)$
	and $ \bar{N}$ is the unipotent radical
	of the opposite parabolic subgroup $ \bar{P}=M\bar{N}$.
	Since $\pi$ is  $\oo$-finite, 
	the  $M_0$-representations  $\pi$ and  $\pi_1$
	factors through $M(\oo_p/p^n)$ for  $n>>0$,
	thus we may extend the representations to  $I(n)$
	via the group homomorphism
	\[
		I(n)\mapsto P(\oo_p/p^n)\to M(\oo_p/p^n).
	\]
	Then when $U_p=K(n)$ there is the isomorphism
	\[
		 \Hom_{\oo[M_0]}(\pi,S_\psi(U^pU_p,E/\oo)^{N_0})=
		 \Hom_{\oo[I(n)]}(\pi,S_\psi(U^pU_p,E/\oo)^{N_0})
		 \cong S_{\pi^{\vee},\psi}(U^pI(n))
	\]
	But $S_{\pi^{\vee},\psi}(U^pI_n)\hookrightarrow 
	S_{\pi_{1}^{\vee},\psi}(U^pI_n)$
	by the previous lemma
	and $\Hom_{\oo[Z_M^+]}(\oo[Z_M],S_{\pi^{\vee},\psi}(U^pI_n)) $
	is isomorphic to (sums) of localizations $S_{\pi^{\vee},\psi}(U^pI_n)$
	by loc.cit,
	we get the desired surjectively on each  $K(n)$.
	The proposition now follows from that
	$\{K(n)\}$ is cofinal in the filtered system $\{U_p\}$
	of all compact open subgroups of $K_p$
	and that the finite  $\oo$-modules
	$\Hom_{\oo[Z_M^+]}(\oo[Z_M],S_{\pi^{\vee},\psi}(U^pI_n)) $
